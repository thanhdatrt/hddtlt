...
@section('content')
    ...
    <div class="card-box mb-30">
        <div class="pd-20">
            <h4 class="text-blue h4">DANH SÁCH KHÓA HỌC</h4>
        ...
        <div class="pb-20">
            <table class="data-table table stripe hover nowrap">
                <thead>
                    <tr>
                        <th  class="table-plus">Mã khóa</th>
                        <th class="datatable-nosort">Tên khóa</th>
                ...
                <tbody>
                    @foreach ($viewkhoahoc as $key => $item)    
                        <tr>
                            <td class="table-plus">{{$item -> makhoa}}</td>
                            <td>{{$item -> tenkhoa}}</td>
                            <td>
                                @if ($item -> status == 1)
                                    <a href="{{URL::to('/hiden_khoahoc'.'/'.$item -> makhoa)}}">
                                        <input type="checkbox" checked name="hienthi" class="switch-btn" data-size="small" data-color="#0099ff">
                                    </a>
                                @else
                                    <a href="{{URL::to('/show_khoahoc'.'/'.$item -> mahe)}}">
                            ...
                            <td>
                                <a href="{{URL::to('/editkhoahoc/'.$item -> makhoa)}}"><i class="dw dw-edit2"></i> Edit</a>
                                <form action="{{URL::to('/deletekhoahoc'.'/'.$item -> makhoa)}}" method="POST">
                                    {{ csrf_field() }}
