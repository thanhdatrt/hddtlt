...
<div class="pd-20 card-box mb-30">
    <div class="clearfix">
        <div class="pull-left">
            <h4 class="text-blue h4">CHỈNH SỬA KHÓA HỌC</h4>
        </div>
    </div>
    </br>
    <?php
    ...
    ?>
    @foreach ($editkhoahoc as $key => $item)
        <form action="{{URL::to('/action_editkhoahoc'.'/'.$item -> makhoa)}}" method="POST">
            {{ csrf_field() }}
            <div class="form-group row">
                <label class="col-sm-6 col-md-2 col-form-label">Mã khóa</label>
                <div class="col-sm-6 col-md-10">
                    <input class="form-control" readonly value="{{$item -> makhoa}}" type="text" name="makhoa" placeholder="Mã khóa...">
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-6 col-md-2 col-form-label">Tên khóa học</label>
                <div class="col-sm-6 col-md-10">
                    <input class="form-control" value="{{$item -> tenkhoa}}" name="tenkhoa" placeholder="Tên khóa..." type="text">
                </div>
            </div>
            <div class="form-group row">
