...
@section('content')
    ...
    <div class="card-box mb-30">
        <div class="pd-20">
            <h4 class="text-blue h4">DANH SÁCH LỚP</h4>
            <div class="pull-right">
                <form class="form-inline" action="{{URL::to('/viewlop')}}" method="GET">
        ...
        <div class="pb-20">
            <table id="tbllop" class="data-table table stripe hover nowrap">
                <thead>
                    <tr>
                        <th  class="table-plus">Mã lớp</th>
                        <th class="datatable-nosort">Tên lớp</th>
                ...
                <tbody>
                    @foreach ($viewlop as $key => $item)    
                        <tr>
                            <td class="table-plus">{{$item -> malop}}</td>
                            <td>{{$item -> tenlop}}</td>
                            <td>
                                @if ($item -> status == 1)
                                    <a href="{{URL::to('/hiden_lop'.'/'.$item -> malop)}}">
                                        <input type="checkbox" checked name="hienthi" class="switch-btn" data-size="small" data-color="#0099ff">
                                    </a>
                                @else
                                    <a href="{{URL::to('/show_lop'.'/'.$item -> malop)}}">
                            ...
                            <td>
                                <a href="{{URL::to('/editlop/'.$item -> malop)}}"><i class="dw dw-edit2"></i> Edit</a>
                                <form action="{{URL::to('/deletelop'.'/'.$item -> malop)}}" method="POST">
                                    {{ csrf_field() }}
