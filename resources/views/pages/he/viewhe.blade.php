...
@section('content')
            ...
            <div class="col-md-6 col-sm-12">
                ...
                <nav aria-label="breadcrumb" role="navigation">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Danh sách hệ</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="card-box mb-30">
        <div class="pd-20">
            <h4 class="text-blue h4">DANH SÁCH HỆ ĐÀO TẠO</h4>
        ...
        <div class="pb-20">
            <table id="tblhe" class="data-table table stripe hover nowrap">
                <thead>
                    <tr>
                        <th  class="table-plus">Mã hệ</th>
                        <th class="datatable-nosort">Tên hệ</th>
                ...
                <tbody>
                    @foreach ($viewhe as $key => $item)    
                        <tr>
                            <td class="table-plus">{{$item -> mahe}}</td>
                            <td>{{$item -> he}}</td>
                            <td>
                                @if ($item -> status == 1)
                                    <a href="{{URL::to('/hiden_he'.'/'.$item -> mahe)}}">
                                        <input type="checkbox" checked name="hienthi" class="switch-btn" data-size="small" data-color="#0099ff">
                                    </a>
                                @else
                                    <a href="{{URL::to('/show_he'.'/'.$item -> mahe)}}">
                            ...
                            <td>
                                <a href="{{URL::to('/edithe/'.$item -> mahe)}}"><i class="dw dw-edit2"></i> Edit</a>
                                <form action="{{URL::to('/deletehe'.'/'.$item -> mahe)}}" method="POST">
                                    {{ csrf_field() }}
